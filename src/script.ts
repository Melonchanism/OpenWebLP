...
function editorInit() {
 ...
 input.addEventListener("keydown", (event: KeyboardEvent) => {
  ...
  if (key === "Enter" && event.ctrlKey) { 
   ...
   localStorage.setItem("songs", JSON.stringify(encodeSongs(input.value)));
   editor.alert("Saved to LocalStorage, not updated");
  } else if (key === "l" && event.ctrlKey) {
   event.preventDefault();
   songs = JSON.parse(localStorage.getItem("songs"));
   if (!songs[song]?.lyrics[lyric]) setSong(0);
   editor.alert("Loaded from LocalStorage, not updated");
   input.value = decodeSongs(songs);
  };
 });
};
...
function configurerInit() {
 ...
 configurer.document.body.innerHTML = `<form><input id="c1" placeholder="display bg color" value="${window.getComputedStyle(preview.contentDocument.body).getPropertyValue("color")}"/><br><input id="c2"/><br><input id="c3"/><br><input id="c4"/><br><input type="submit" value="Update"/></form>`;
 const form = configurer.document.querySelector("form")
 form.addEventListener("submit", (event: SubmitEvent) => { 
  event.preventDefault();
  configurer.alert("Updated");
 });
}
 ...
 [document, preview.contentDocument].forEach(item => {
  item.addEventListener("keydown", (event: KeyboardEvent) => {
   ...
   if (key === "d") {
    ...
    editor = open(`about:blank`, "_blank");
