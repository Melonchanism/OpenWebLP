...
let display: Window | null,
 ...
 config = {
  ...
  editor: `<title>OpenWebLP Editor</title><style>body{background-color:black;color:white;font-family:sans-serif;margin:0px;overflow:hidden}textarea{margin:0px;border:none;padding:0px;background-color:black;width:100vw;height:100vh;color:white}</style><link rel="icon" href=${icon}>`,
 };
 ...
 [document, preview.contentDocument].forEach(item => {
  item.addEventListener("keydown", (event: KeyboardEvent) => {
   ...
   if (key === "d") {
    ...
    if (browser === "firefox") {
     display.addEventListener("load", displayInit);
    ...
    if (browser === "firefox") {
     editor.addEventListener("load", editorInit);
