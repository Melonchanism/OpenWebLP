...
function updateSettings() {
	settings = JSON.parse(localStorage.getItem("settings") ?? `{"transition": "fade", "info": false}`);
	animationName = settings.transition;
}
</script>

<svelte:window on:keydown={evt => evt.key !== "f" ? channel.postMessage([evt.key, evt.shiftKey]) : document.body.requestFullscreen()} />
